...
struct tu_type_function_mismatch_data {
  struct tu_source_location location;
  struct tu_type_descriptor * type;
};
...
extern "C"
{
    ...
    {
        printk("tinyubsan: %s at file %s, line %d, column %d\n", message, loc.file, loc.line, loc.column);
        arch_die();
    }
    ...
    void __ubsan_handle_function_type_mismatch(struct tu_type_function_mismatch_data * data, uintptr_t __unused ptr) {
        tu_print_location("function type mismatch\n", data->location);
    }
    ...
    void __ubsan_handle_type_mismatch(struct tu_type_mismatch_v1_data *data, uintptr_t ptr)
    {
        if (!ptr)
        {
